...
ggplot(totchgagewise)+
  geom_point(aes(x = AGE, y = summary(Healthcare$AGE),size  = TOTCHG),color ='blue',shape = 22 ,fill = 'black')+
  theme(plot.background = element_rect(fill = 'white'),panel.background = element_rect(fill = 'orange'))+
  xlab('Age Group')+
  ylab('Counts')+
...
#Q5 whether LOS is predictable from age ,gender and race 
#lmtest
Healthcare$LOS = as.numeric(Healthcare$LOS)
str(Healthcare)
fit__1 = lm(LOS~AGE+FEMALE+RACE,Healthcare)
fit__1
summary(fit__1)
#|______________> Hence length of stay cannot be predicted from AGE,GENDER,RACE.
